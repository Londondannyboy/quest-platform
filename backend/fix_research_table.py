"""
Fix article_research table
"""
import asyncio
import asyncpg
import os

async def fix_research_table():
    db_url = os.getenv("NEON_CONNECTION_STRING") or os.getenv("DATABASE_URL")
    conn = await asyncpg.connect(db_url)

    try:
        print("Dropping and recreating article_research table...")
        ...
        await conn.execute("CREATE INDEX idx_research_expires ON article_research(expires_at) WHERE expires_at > NOW()")
...
if __name__ == "__main__":
    asyncio.run(fix_research_table())
